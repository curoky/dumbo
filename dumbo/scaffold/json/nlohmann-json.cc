...
#include <nlohmann/json.hpp>  // for basic_json, basic_json<>::object_t, operator==, operator<<, basic_json<>::type_error, basic_json<>::value_type, json
...
#include <string>            // for string, allocator, operator==, hash, equal_to
...
namespace tbb {
void to_json(json& j, const concurrent_unordered_set<std::string>& p) {}

void from_json(const json& j, concurrent_unordered_set<std::string>& p) {}
}  // namespace tbb

TEST_CASE("[Nlohmann-json]: tbb to/from json") {
  json j;
  {
  ...
  {
    tbb::concurrent_unordered_set<std::string> set;
    j["set"] = set;
  }
  {
